...
class ItemContactInfoViewController: UIViewController {
    ...
    @IBAction func pressSave(_ sender: UIButton) {
        ...
        let newItem = ["name": self.item.itemTitle,
                       "notes": self.item.notes,
                       "loanee": self.item.loanee?.contactInfo.identifier]
        ...
        uploadProfileImage(item.itemImage, key: key!) { (url) in
            print(url ?? "no url bro")
            ref.child("items").child(key!).updateChildValues(["imageUrl": url?.absoluteString ?? "No URL Provided"])
